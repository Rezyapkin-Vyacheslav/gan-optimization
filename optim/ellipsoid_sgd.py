...
class EllipsoidSGD(Optimizer):
    ...
    def __init__(self, params, H, lr=required, momentum=0, dampening=0,
        ...
        super(EllipsoidSGD, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(EllipsoidSGD, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group, group_H in zip(self.param_groups, self.H):
            weight_decay = group['weight_decay']
            momentum = group['momentum']
            dampening = group['dampening']
            nesterov = group['nesterov']

            for p, H in zip(group['params'], group_H):
                if p.grad is None:
                    continue
                if H.sum() < 1e-12:
                    continue
                ...
                if DEBUG>0:
                    ...
                    print('Gradient: ')
                    print(d_p)
#                 Hw = H.mm(d_p.view(-1, 1))
#                 wHw = d_p.view(1, -1).mm(Hw)
#                 p.add_(Hw.div(wHw.sqrt()).view_as(p), alpha=-1/(n+1))
#                 H.add_(torch.ger(Hw.view(-1), Hw.view(-1)).div(wHw), alpha=-2/(n+1))
#                 H.mul_(n**2/(n**2-1))
                denom_H = (d_p.view(1, -1).mm(H)).mm(d_p.view(-1, 1))
                if DEBUG>1:
                    print("Denominator shape: ", denom_H.size())
                p.add_(H.mm(d_p.view(-1, 1)).div(denom_H.sqrt()).view_as(p), alpha=-1/(n+1)*group['lr'])
                H.add_(H.mm(d_p.view(-1, 1)).mm(d_p.view(1,-1)).mm(H).div(denom_H), alpha=-2/(n+1)*group['lr'])
                H.mul_(n**2/(n**2-1))
        return loss